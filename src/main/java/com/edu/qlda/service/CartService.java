...
public class CartService {
    ...
    public void addProductToCart(CartrequestDto request) {
        ...
        Optional<Cart> existingCartItem = cartRepository.findByCartIdAndProductId(
                request.getEmployeeId(), request.getProductId());

        if (existingCartItem.isPresent()) {
            int newQuantity = existingCartItem.get().getQuantity() + request.getQuantity();
            cartRepository.updatecart(newQuantity, request.getEmployeeId(), request.getProductId());
        } else {
    ...
    public void deleteCartItems(List<Integer> cartItemIds) {
        if (cartItemIds == null || cartItemIds.isEmpty()) {
            throw new ValidationException("Cart item list must not be empty.");
        }
